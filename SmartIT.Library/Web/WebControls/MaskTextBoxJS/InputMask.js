...
function InputMask(fields, control, keyPressFunction, keyDownFunction, keyUpFunction, blurFunction, updateFunction, changeFunction) {
    if (isInstance(fields, String)) { fields = maskBuilder.parse(fields) } else if (isInstance(fields, MaskField)) { fields = [fields] }
    ...
    this.control.mask = this; this.control.pad = false; this.control.ignore = false; this.keyDownFunction = keyDownFunction || null; this.keyPressFunction = keyPressFunction || null; this.keyUpFunction = keyUpFunction || null; this.blurFunction = blurFunction || null; this.updateFunction = updateFunction || null; function onKeyDown(event) {
    ...
    observeEvent(this.control, "focus", onFocus); function onBlur(event) {
        ...
        if (this.mask.changeFunction != null) {
            if (this._lastValue != this.value && this.mask.changeFunction != null) {
                var e = {}; for (property in event) { e[property] = event[property] }
    ...
    observeEvent(this.control, "blur", onBlur); this.isComplete = function () {
        applyMask(this, true); if (this.fieldValues == null && ((this.fields != null) || (this.fields.length > 0))) { return false }
    ...
    this.isOnlyFilter = function () {
        if (this.fields == null || this.fields.length == 0) { return true }
    ...
    this.transformsCase = function () {
        if (this.fields == null || this.fields.length == 0) { return false }
...
function NumberMask(parser, control, maxIntegerDigits, allowNegative, keyPressFunction, keyDownFunction, keyUpFunction, blurFunction, updateFunction, leftToRight, changeFunction) {
    if (!isInstance(parser, NumberParser)) { alert("Illegal NumberParser instance"); return }
    ...
    observeEvent(this.control, "keydown", onKeyDown); function onKeyPress(event) {
        ...
        this.oldValue = this.value; if (typedChar == '-') {
            if (this.mask.allowNegative) { this.swapSign = true }
    ...
    observeEvent(this.control, "keypress", onKeyPress); function onKeyUp(event) {
        if (this.mask.parser.decimalDigits < 0 && !this.mask.leftToRight) { alert("A NumberParser with unlimited decimal digits is not supported on NumberMask when the leftToRight property is false"); this.value = ""; return false }
        if (window.event) { event = window.event }
    ...
    this.getAsNumber = function () {
        var number = this.parser.parse(this.control.value); if (isNaN(number)) { number = null }
        return number
    }
    this.setAsNumber = function (number) {
        var value = ""; if (isInstance(number, Number)) { value = this.parser.format(number) }
...
function DateMask(parser, control, validate, validationMessage, keyPressFunction, keyDownFunction, keyUpFunction, blurFunction, updateFunction, changeFunction) {
    if (isInstance(parser, String)) { parser = new DateParser(parser) }
    ...
    dateMask.showValidation = true; if (dateMask.extraKeyPressFunction != null) { var ret = invokeAsMethod(this, dateMask.extraKeyPressFunction, [event, dateMask]); if (ret == false) { return false } }
        return true
    }
    this.extraBlurFunction = blurFunction || null; function maskBlurFunction(event, dateMask) {
        var control = dateMask.control; if (dateMask.validate && control.value.length > 0) {
            if (!dateMask.parser.isValid(control.value)) {
                var msg = dateMask.validationMessage; if (dateMask.showValidation && !isEmpty(msg)) { dateMask.showValidation = false; msg = replaceAll(msg, "${value}", control.value); msg = replaceAll(msg, "${mask}", dateMask.parser.mask); alert(msg) }
    ...
    var fields = []; var old = ''; var mask = this.parser.mask; while (mask.length > 0) {
        var field = mask.charAt(0); var size = 1; var maxSize = -1; var padFunction = null; while (mask.charAt(size) == field) { size++ }
        mask = mid(mask, size); switch (field) {
            case 'd': case 'M': case 'h': case 'H': case 'm': case 's': maxSize = 2; break; case 'y': padFunction = getFullYear; if (size == 2) { maxSize = 2 } else { maxSize = 4 }
    ...
    this.setAsDate = function (date) {
        var value = ""; if (isInstance(date, Date)) { value = this.parser.format(date) }
        this.control.value = value; this.update()
    }
}
function SizeLimit(control, maxLength, output, outputText, updateFunction, keyUpFunction, blurFunction, keyDownFunction, keyPressFunction, changeFunction) {
    control = getObject(control); if (!isValidControlToMask(control)) { alert("Invalid control to limit size"); return } else { this.control = control; prepareForCaret(control) }
    if (!isInstance(maxLength, Number)) { alert("Invalid maxLength"); return }
    this.control = control; this.maxLength = maxLength; this.output = output || null; this.outputText = outputText || JST_DEFAULT_LIMIT_OUTPUT_TEXT; this.updateFunction = updateFunction || null; this.keyDownFunction = keyDownFunction || null; this.keyPressFunction = keyPressFunction || null; this.keyUpFunction = keyUpFunction || null; this.blurFunction = blurFunction || null; this.control.sizeLimit = this; function onKeyDown(event) {
        if (window.event) { event = window.event }
...
function applyMask(mask, isBlur) {
    var fields = mask.fields; if ((fields == null) || (fields.length == 0)) { return }
    var control = mask.control; if (control.value == "" && isBlur) { return true }
    var out = ""; if (mask.isOnlyFilter()) {
        var field = fields[0]; for (var i = 0; i < control.value.length; i++) { var c = control.value.charAt(i); if (field.isAccepted(c)) { out += c } }
        control.value = field.transformValue(out); if (isBlur && control.sizeLimit) {
            control.sizeLimit.update()
        }
        return false
    }
    ...
    for (var i = 0; i < fields.length; i++) {
        var field = fields[i]; if (field.literal) {
            if (i > 0) { if (fields[i - 1].max == -1) { var descriptor = {}; descriptor.field = field; descriptor.position = value.indexOf(field.text); if (descriptor.position >= 0) { fixedPositionLiterals[fixedPositionLiterals.length] = descriptor } } }
    ...
    var pos = 0; for (var i = 0; (i < fields.length) && (pos < value.length); i++) {
        var field = fields[i]; if (field.literal) {
            var fixed = false; for (var j = 0; j < fixedPositionLiterals.length; j++) { var descriptor = fixedPositionLiterals[j]; if (descriptor.field.text == field.text) { fixed = true; break } }
            if (!fixed) { out += field.text; if (control.caretPosition != null) { control.caretPosition += field.text.length; typedIndex-- } }
        } else {
            var upTo = field.upTo(value, pos); if (upTo == -1) { break } else {
                var fieldValue = field.transformValue(value.substring(pos, upTo + 1)); if (control.pad) { fieldValue = field.pad(fieldValue) }
                var completeField = field.isComplete(fieldValue); if (control.supportsCaret && !control.pad && i < fields.length - 1 && upTo == typedIndex - 1) {
                    var nextField = fields[i + 1]; if (nextField.literal && nextField.text.charAt(0) == typedChar) {
                        if (!completeField) { fieldValue = field.pad(fieldValue) }
...
function applyNumberMask(numberMask, isBlur) {
    var control = numberMask.control; var value = control.value; if (value == "") { return true }
    ...
    var pos = value.indexOf(parser.decimalSeparator); var hasDecimal = (pos >= 0); var caretAdjust = 0; if (leftToRight) {
        if (hasDecimal) { intPart = value.substr(0, pos); decPart = value.substr(pos + 1) } else { intPart = value }
        if (isBlur && parser.decimalDigits > 0) { decPart = rpad(decPart, parser.decimalDigits, '0') }
    } else { var decimalDigits = parser.decimalDigits; value = replaceAll(value, parser.decimalSeparator, ''); intPart = left(value, value.length - decimalDigits); decPart = lpad(right(value, decimalDigits), decimalDigits, '0') }
    ...
    if ((parser.useGrouping) && (!isEmpty(parser.groupSeparator))) {
        var group, temp = ""; for (var i = intPart.length; i > 0; i -= parser.groupSize) { group = intPart.substring(intPart.length - parser.groupSize); intPart = intPart.substring(0, intPart.length - parser.groupSize); temp = group + parser.groupSeparator + temp }
        intPart = temp.substring(0, temp.length - 1)
    }
    var out = new StringBuffer(); var oneFormatted = parser.format(isNegative ? -1 : 1); var appendEnd = true; pos = oneFormatted.indexOf('1'); out.append(oneFormatted.substring(0, pos)); out.append(intPart); if (leftToRight) { if (toDecimal || !isEmpty(decPart)) { out.append(parser.decimalSeparator).append(decPart); appendEnd = !toDecimal } } else {
        if (parser.decimalDigits > 0) { out.append(parser.decimalSeparator) }
        out.append(decPart)
    }
    ...
    control.value = out.toString(); if (caret != null && !isBlur) {
        if (!hadSymbol && ((value.indexOf(parser.currencySymbol) >= 0) || (value.indexOf(parser.decimalSeparator) >= 0))) { caretToEnd = true }
...
function checkSizeLimit(control, isBlur) {
    var sizeLimit = control.sizeLimit; var max = sizeLimit.maxLength; var diff = max - control.value.length; if (control.value.length > max) { control.value = left(control.value, max); setCaretToEnd(control) }
...
this.base = MaskField; this.base(); this.accepted = accepted; if (min != null && max == null) { max = min }
    ...
    this.upTo = function (text, fromIndex) {
        text = text || ""; fromIndex = fromIndex || 0; if (text.length < fromIndex) { return -1 }
    ...
    this.isComplete = function (text) {
        text = String(text); if (text.length < this.min) { return false }
    ...
    this.pad = function (text) {
        text = String(text); if (((this.max >= 0) || (text.length <= this.max)) || this.max < 0) {
            var value; if (this.padFunction != null) { value = this.padFunction(text, this.min, this.max) } else { value = text }
            if (this.max > 0) {
                var padChar = ' '; if (this.accepted == null || this.accepted.indexOf(' ') > 0) { padChar = ' ' } else if (this.accepted.indexOf('0') > 0) { padChar = '0' } else { padChar = this.accepted.charAt(0) }
...
function MaskBuilder() {
    this.parse = function (string) {
        if (string == null || !isInstance(string, String)) { return this.any() }
        ...
        for (var i = 0; i < string.length; i++) {
            var c = string.charAt(i); if (start == null) { start = i }
            ...
            if (lastType != type && lastType != null) { var text = string.substring(start, i); fields[fields.length] = switchField(lastType, text); start = i; lastType = type } else {
                lastType = type
            }
        }
